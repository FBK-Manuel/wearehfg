...
import type { itemProps, ProductCardProps, replatedProductProps } from "../dataType/DataType";
...
const fetchProducts = async (productId: number) => {
  try {
    const res = await api.get(`/relatedProducts.php?id=${productId}`);
...
// -------------------- Today's Deal --------------------
const RelatedProduct: React.FC<replatedProductProps> = ({productId}) => {
  const {
    data: products,
    isLoading,
    isError,
  } = useQuery<ProductCardProps[], Error>({
    queryKey: ["products", 8],
    queryFn: () => fetchProducts(productId),
    retry: 1,
  });

  const finalProducts = isError ? dummyProducts : (products ?? dummyProducts);
    ...
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 w-full bg-transparent">
        {isLoading ? (
          ...
          finalProducts.map((p: ProductCardProps) => (
            <RelatedItem key={p.id} {...p} />
          ))
        )}
      </div>
    </>
